"""
FastMCP server implementation for Ergo blockchain payments.
"""
...
# Try to determine the port immediately
try:
    sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    sock.bind(('', 0))
    port = sock.getsockname()[1]
    sock.close()
    print(f"MCP Server will run on port: {port}")
    logger.info(f"MCP Server will run on port: {port}")
except Exception as e:
    logger.error(f"Failed to determine port: {str(e)}")
