...
def dispense_tokens(appKit: ErgoAppKit, token_id: str, recipient_addresses: List[str], tokens_per_round: int, proxy_contract: bytes) -> None:
    ...
    # Fetch all unspent boxes for the proxy contract
    all_unspent_boxes = appKit.getUnspentBoxes(proxy_address, total_erg_needed)
...
def recover_erg(appKit: ErgoAppKit, proxy_contract: bytes, node_address: str) -> None:
    proxy_address = Address.fromErgoTree(proxy_contract, appKit._networkType).toString()
    ...
    logger.info(f"Recovered {total_erg/ERG_TO_NANOERG:.9f} ERG and {total_tokens} tokens")
...
class ErgoTokenDistributor:
    ...
    def recover(self) -> None:
        recover_erg(self.appKit, self.proxy_contract, self.config.node_address)
