...
class MultiOutputBuilder:
    ...
    def calculate_required_amounts(self, outputs: List[OutputBox]) -> Tuple[int, Dict[str, int]]:
        ...
        Returns:
            Tuple[int, Dict[str, int]]: (total_erg_nanoerg, token_amounts)
        """
    ...
    def select_boxes(self, address: str, required_erg: int, required_tokens: Dict[str, int]) -> BoxSelection:
        ...
        Uses a greedy algorithm with consideration for box consolidation.
        """
        try:
    ...
    def validate_transaction(self, outputs: List[OutputBox], sender_address: str) -> Tuple[bool, str]:
        """Validate if the transaction can be executed with current wallet balances."""
        try:
            required_erg, required_tokens = self.calculate_required_amounts(outputs)
            try:
                self.select_boxes(sender_address, required_erg, required_tokens)
